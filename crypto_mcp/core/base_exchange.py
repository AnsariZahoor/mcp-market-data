...
# Module-level shared HTTP client for all exchange instances
_EXCHANGE_CLIENT: Optional[httpx.Client] = None


def _get_exchange_client() -> httpx.Client:
    """
    Get or create shared HTTP client for exchange API calls.
    Reused across all exchange instances for optimal performance.
    """
    global _EXCHANGE_CLIENT
    if _EXCHANGE_CLIENT is None:
        # Configure connection pool for multiple exchanges
        limits = httpx.Limits(
            max_keepalive_connections=30,  # More connections for multiple exchanges
            max_connections=100,            # Higher limit for parallel requests
            keepalive_expiry=60.0           # Keep idle connections for 60s
        )
        
        _EXCHANGE_CLIENT = httpx.Client(
            timeout=30.0,
            limits=limits,
            http2=True  # Enable HTTP/2 for multiplexing
        )
        logger.info("Initialized shared exchange HTTP client with connection pooling (max_keepalive=30, max_conn=100)")
...
class BaseExchange(ABC):
    ...
    def __init__(self, db_handler=None, cache_ttl: int = 60):
        ...
        # Use shared client for connection pooling across all exchange instances
        self._client = _get_exchange_client()
        self.cache_ttl = cache_ttl
        self._cache: Dict[str, Tuple[float, Dict]] = {}  # {market_type: (timestamp, data)}

    @property
    def client(self) -> httpx.Client:
        """Return shared HTTP client (connection pooling enabled)"""
        return self._client

    def close(self):
        """No-op: shared client is not closed per-instance"""
        pass
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        """Context manager exit - shared client remains open for reuse"""
        return False

    def __del__(self):
        """Cleanup fallback - no-op for shared client"""
        pass
